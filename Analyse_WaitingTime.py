...
def AnalyseWaitingTime():
    ...
    dataGeneration()
    simulation_default,simulation_anti = runEpisode() # 2 simulation results

    # get avg waiting time per order
    def compute_avg(ls):
        if ls:
            return round(sum(ls)/len(ls), 3)
        else:
            print("empty list")
    
    avg_wt_default = compute_avg(simulation_default.wt_ls)
    avg_wt_anti = compute_avg(simulation_anti.wt_ls)
