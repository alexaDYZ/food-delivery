...
def assign_order_to_rider(order: Order, rider_list, currTime:float) -> int: # returns the index of the best rider
    ...
    # select the nearest rider(to restaurant)
        ...
        bestRiderId = bestRider.id # nearest rider is the best rider
        print("best rider is ", bestRiderId)
        # update rider status
        bestRider.deliver(order, currTime)
        # update order status
        order.assignRider(bestRiderId)
