...
class AssignLaterMethod_ImperfectKnowledge(AssignmentMethod):
    def __init__(self):
        super().__init__()
        ...
        self.threshold = None 
        # self.FPT_predicted = args["FPT_pred"]
        self.FPT_predicted = None
    
    def setFPT_pred(self, FPT_pred):
        self.FPT_predicted = FPT_pred

    def addOrder(self, newOrder):
        super().addOrder(newOrder)
    ...
    def findR2RforAll(self):
        ...
        def getTimeReachedRestaurant(rider:Rider, currTime): 
            ...
            t_start = max(currTime, rider.nextAvailableTime_predicted)
            return round(t_start + R2R,3)

        for r in self.rider_list:
            arrival_time = getTimeReachedRestaurant(r, currTime)
            if arrival_time in RiderArriveTime_pred:
                RiderArriveTime_pred[arrival_time].append(r)
            else:
                RiderArriveTime_pred[arrival_time] = [r]
        
        

        return RiderArriveTime_pred

    # driver function, called in Event.py
    def find_best_rider(self):
       
        self.setFPT_pred(args["FPT_pred"])
        self.order.FPT_predicted = self.FPT_predicted
        ...
        # Case 2. otherwise, it is a new order
            ...
            diff_pred = self.find_FPT_pred_minus_threshold()
            ...
            # 2.2 FPT is large, assign later
                ...
                self.order.reassign_time = self.currTime + diff_pred
    ...
    def assignNow(self):
        ...
        t_arrive_restaurant_actual = t_arrive_restaurant_pred - bestRider.nextAvailableTime_predicted + t_start_actual
    ...
    def find_FPT_pred_minus_threshold(self):
        # output = FPT - threshold
        # output > 0 means FPT is later than the threshold, hence assign later
        # output <= 0 means FPT is fast enough to be assigned now
        FPT_pred = self.order.FPT_predicted
        threshold = args["threshold_assignment_time"]
        res = round(FPT_pred - threshold, 3)
        return res
...
class AssignLaterMethod_UsefulWork(AssignLaterMethod_ImperfectKnowledge):
    def __init__(self):
        super().__init__()
